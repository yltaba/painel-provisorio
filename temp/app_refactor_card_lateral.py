...
pib_corrente = calcular_pib_atual(pib_por_categoria)
variacao_pib = calcular_variacao_pib(pib_por_categoria)

card_pib_corrente = dbc.Card(
    dbc.CardBody(
        [
            html.H5(f"PIB {pib_por_categoria["ano"].max()}", className="card-title"),
            html.Div(
                [
